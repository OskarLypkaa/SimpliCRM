...
struct WeekBasicView: View {
    ...
    var body: some View {
        HStack(alignment: .top) {
            ...
            // Siatka komórek
            DayGrid(firstRowDate: firstRowDate)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ForEach(6..<23, id: \.self) { hour in
                Text("\(hour):00")
                    .frame(height: 30) // Dopasowanie wysokości do komórek po prawej stronie
            }
        }
        .frame(width: 50)
...
struct DayGrid: View {
    ...
    private func isCurrentTime(in column: Int, at row: Int) -> Bool {
        ...
        // Wiersz odpowiada bieżącej godzinie i minucie (30-minutowe okna)
        let rowHour = 6 + (row / 2) // 6:00 to początek
        let rowMinuteSlot = (row % 2) * 30

        return rowHour == currentHour && currentMinute >= rowMinuteSlot && currentMinute < rowMinuteSlot + 30
    }
}
